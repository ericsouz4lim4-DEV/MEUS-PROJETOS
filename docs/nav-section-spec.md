...
### Stacking / blur (tokens em `:root`, inferidos do checklist §7 + microinterações em DESIGN.md)
| Propriedade | Token | Nota |
| :--- | :--- | :--- |
| **Stacking** | `--z-base`, `--z-floating-nav`, `--z-modal` | Nav acima do conteúdo; `--z-modal` reservado para modais de trailer. |
| **Desfoque opcional da pill** | `--nav-backdrop-blur` | `backdrop-filter` discreto sobre o conteúdo em scroll (`transform`/`opacity` no motion). |
